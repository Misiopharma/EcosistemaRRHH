...
// Lógica para establecer días no laborales
$fecha_actual = date('Y-m-d');
$dia_semana = date('w', strtotime($fecha_actual)); // 0 (para domingo) hasta 6 (para sábado)

if ($dia_semana == 0 || $dia_semana == 6) { // Si es sábado (6) o domingo (0)
    $empleados = $pdo->query("SELECT id FROM empleados")->fetchAll(PDO::FETCH_ASSOC);
    ...
    foreach ($empleados as $empleado) {
        ...
        if ($asistencia) {
            // Si ya existe un registro, actualizamos el estado a "Día no laboral"
            $stmt = $pdo->prepare("UPDATE asistencia SET estado = 'Día no laboral' WHERE id = ?");
            $stmt->execute([$asistencia['id']]);
        } else {
            // Si no existe un registro, lo creamos con el estado "Día no laboral"
...
foreach ($empleados as $empleado) {
    ...
    if (!$asistencia) {
        // Si no hay registro de entrada, marcar como ausente
        $stmt = $pdo->prepare("INSERT INTO asistencia (empleado_id, fecha, estado, justificacion) VALUES (?, ?, 'Ausente', 'Sin justificación')");
        $stmt->execute([$empleado['id'], $fecha_actual]);
    }
}
