...
$hoy = date('Y-m-d');
$sqlPresentes = "SELECT COUNT(*) AS presentes_hoy FROM asistencia WHERE fecha = :hoy AND estado = 'presente'";
...
$stmtAusentes = $conn->prepare($sqlAusentes);
$stmtAusentes->bindParam(':hoy', $hoy);
$stmtAusentes->execute();
$ausentesHoy = $stmtAusentes->fetch(PDO::FETCH_ASSOC)['ausentes_hoy'];

$sqlTardanzas = "SELECT COUNT(*) AS tardanzas_hoy FROM asistencia WHERE fecha = :hoy AND estado = 'tardanza'";
$stmtTardanzas = $conn->prepare($sqlTardanzas);
...
$stmtInasistencias = $conn->prepare($sqlInasistencias);
